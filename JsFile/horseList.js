class HorseCard {
    ...
    createCardElement() {
        ...
        horseContent.classList.add("horse-content");
        card.appendChild(horseContent);

        const image = document.createElement('img');
        image.src = this.horse.image;
        image.alt = this.horse.name;
        imageContainer.appendChild(image);

        const name = document.createElement('h2');
        name.classList.add("name");
        name.textContent = this.horse.name;
        horseContent.appendChild(name);

        const detailsButton = document.createElement("a");
        detailsButton.classList.add("details-link");
        detailsButton.href = `./horse_details.html?id=${this.id}`;
        detailsButton.textContent = "View Details"
        horseContent.appendChild(detailsButton)
...
horses.forEach(horse => {
    const horseCard = new HorseCard(horse);
    const cardElement = horseCard.createCardElement();
